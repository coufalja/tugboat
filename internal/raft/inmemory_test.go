...
func TestRateLimited(t *testing.T) {
	tests := []struct {
		...
		{nil, false},
		{rate.NewInMemRateLimiter(0), false},
		{rate.NewInMemRateLimiter(math.MaxUint64), false},
		{rate.NewInMemRateLimiter(1), true},
		{rate.NewInMemRateLimiter(math.MaxUint64 - 1), true},
	}
	for idx, tt := range tests {
...
func TestRateLimitClearedAfterRestoringSnapshot(t *testing.T) {
	im := newInMemory(0, rate.NewInMemRateLimiter(10000))
...
func TestRateLimitIsDecreasedAfterEntriesAreApplied(t *testing.T) {
	...
	im := newInMemory(2, rate.NewInMemRateLimiter(10000))
	im.merge(ents)
